...
class UserRoutes {
    ...
    initRoutes() {
        ...
        this.router.get(
            ...
            this.authService.isLoggedIn,
            this.authService.isAdmin,
            param('role').isIn(["Customer", "Manager", "Admin"]),
            this.errorHandler.validateRequest,
        ...
        this.router.delete(
            "/:username",
            this.authService.isLoggedIn,
            param('username').notEmpty().withMessage("user is empty"),
            this.errorHandler.validateRequest,
        ...
        this.router.patch(
            ...
            body('birthdate').notEmpty().isDate().matches(/^\d{4}-\d{2}-\d{2}$/),
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) => {
                this.controller.getUserByUsername(req.user, req.params.username)
                  .then(async (user) => {
                    ...
                    // Validate Birthdate (if birthdate is required for update)
                    if (req.body.birthdate) {
                      const birthdate = dayjs(req.body.birthdate);
                      if (birthdate.isAfter(dayjs())) {
                        return res.status(400).json({ message: "Birthdate cannot be in the future" });
                      }
                    }
              
                    // Check for Authorization and User Modification (if applicable)
                    if (user.role === 'Admin' && req.user.role === 'Admin') {
                      return res.status(401).json({ message: "Unauthorized: Admins cannot modify other admins" });
                    }
              
                    // Update User Information (if all validations pass)
                    return this.controller.updateUserInfo(req.user, req.body.name, req.body.surname, req.body.address, req.body.birthdate, req.params.username);
                  })
