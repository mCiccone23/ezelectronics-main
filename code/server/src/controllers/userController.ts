...
import { UnauthorizedUserError, UserNotAdminError, UserIsAdminError, UserNotFoundError } from "../errors/userError"
...
class UserController {
    ...
    async deleteUser(user: User, username: string) /**:Promise<boolean>*/ {
        if (user.role !== "Admin" && user.username !== username) throw new UserNotAdminError()
        if (user.role === "Admin" && user.username === username) throw new UserIsAdminError()
        const userToDel = await this.dao.getUserByUsername(username);
        if (!userToDel) throw new UserNotFoundError()
        if (userToDel.role === "Admin") throw new UserIsAdminError()
